...
public class Query<T extends DbModel> {
     ...
     * @param json a JSON object containing a query
    ...
    /**
     * Same as fromJson(), but expects a URL-encoded set of parameters for the query.
     * @param collection the collection that the query will be run against
     * @param params a string containing a URL-encoded query
     * @param <T> the model type
     * @return the query object
     */
    public static <T extends DbModel> Query<T> fromParams(DbCollection<T> collection, String params) {
        JsonObject json = new ParamsParser(params).toJson();
        return Query.fromJson(collection, json);
    }
